...
X[X$step == "FindAllMarkers","time"] <- X[X$step == "FindAllMarkers","time"] / X[X$step == "FindAllMarkers","clusters"] * 30

ols <- lm(time~size:implementation:step + implementation:step + I(size^2):implementation:step - 1, X)
summary(ols)

Z <- X %>% dplyr::group_by(size, implementation, step) %>%
            dplyr::summarize(mu=mean(time, na.rm=T), ci=2*sd(time, na.rm=T)) %>%
            dplyr::ungroup()
...
YS <- Y %>%
        dplyr::group_by(size, implementation) %>%
            dplyr::summarize(count=sum(!is.na(totaltime)), ci=2*sd(totaltime, na.rm=T), totaltime=mean(totaltime, na.rm=T), clusters=median(clusters)) %>%
        dplyr::ungroup()

ols <- lm(totaltime~size:implementation + implementation + I(size^2):implementation - 1, Y)
#ols <- lm(t~size:implementation + implementation - 1, Y)
YS <- cbind(YS, predict=predict(ols, YS))

ggplot(YS, aes(x=size,y=totaltime, fill=implementation, color=implementation)) + geom_pointrange(aes(ymin=(totaltime-ci), ymax=(totaltime+ci)), position=pd) + x_scale + guides(x =  guide_axis(angle = 45))
